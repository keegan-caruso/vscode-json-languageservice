...
export function isKeywordEnabled(
	...
	const VOCABULARY_KEYWORDS: { [uri: string]: string[] } = {
		// JSON Schema 2019-09 vocabularies
	...
	// If no vocabulary constraints, all keywords are enabled
	if (!activeVocabularies) {
		return true;
	}

	// Check if this keyword belongs to any active vocabulary
	for (const [vocabUri, keywords] of Object.entries(VOCABULARY_KEYWORDS)) {
	...
	const core201909 = VOCABULARY_KEYWORDS['https://json-schema.org/draft/2019-09/vocab/core'];
	const core202012 = VOCABULARY_KEYWORDS['https://json-schema.org/draft/2020-12/vocab/core'];
